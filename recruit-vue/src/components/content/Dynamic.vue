<template>
  <div style="position: relative">
    <div style="transition: all 5s;display: flex;justify-content: center;flex-wrap: wrap">
      <div v-if="!dynamicShow">
        <i  class="el-icon-loading"></i>正在拼了老命加载...
      </div>

      <div v-if="dynamicShow" class="dynamic" v-for="(item,index) in page.records" :key="item.cid">
        <div style="box-sizing: border-box;padding:10px"  >
          <header>
            ...
            <div id="user">
              <div   style="display: flex;flex-direction: column;justify-content: space-around">
                <div class="userName" >
                  <h3 class="userName2" @click="toArchive(item.uid,item.type)">{{getName(item.uid,item.type)}}</h3>
                  <user-card :users="item" class="userCard"  style="position: absolute;left: 80%;top:-60px;z-index: 5"></user-card>
                </div>

                <p style="color: rgb(135,137,135)">{{getTime(item.createTime)}}</p>
              </div>
          ...
          <article>
            ...
            <div style="display: flex;width:89%;flex-direction: column;justify-content: space-between">
              <div style="word-break: break-all">{{item.context}}</div>
        ...
        <footer>
          ...
          <div @click="getComment(item.id)" class="fBtn"><i class="el-icon-s-comment"> </i> {{getCommentNum(item.id)}}
    ...
    <el-button v-if="isMore" style="width: 100%" @click="getMore()">获取更多</el-button>
